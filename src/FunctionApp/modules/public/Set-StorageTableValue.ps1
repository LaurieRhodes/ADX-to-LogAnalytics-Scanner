function Set-StorageTableValue {
    Param (
        [string]$ClientId,
...
<#
  ...
  Renamed from Push-StorageTableValue to comply with PowerShell approved verbs.
#>

    $resource = "https://storage.azure.com/"

    Write-Debug "(Set-StorageTableValue) calling Get-AzureADToken -resource $($resource) -clientId $($ClientId)"
    ...
    try {
        Write-Debug "(Set-StorageTableValue) calling-Uri $($uri )"
        $response = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method PUT -Body $body
        Write-Debug "(Set-StorageTableValue) response.value received $($response.value)"
